...
import tensorflow.compat.v1 as tf
if os.environ.get("EVAL_DEVICE", "").lower() == "cpu":
...
def main():
    ...
    if IS_NPU_AVAILABLE:
        ...
        print("Enabled NPU acceleration for the InceptionV3 graph.")
    else:
        config.gpu_options.allow_growth = True
...
class Evaluator:
    def __init__(
        ...
        with self.sess.graph.as_default():
            ...
            self.pool_features = _create_feature_graph(self.image_input)
            self.softmax = _create_softmax_graph(self.softmax_input)

    def warmup(self):
        self.compute_activations(np.zeros([1, 8, 64, 64, 3]))

    def read_activations(self, npz_path: str) -> Tuple[np.ndarray]:
        with open_npz_array(npz_path, "arr_0") as reader:
            return self.compute_activations(reader.read_batches(self.batch_size))

    def compute_activations(self, batches: Iterable[np.ndarray]) -> Tuple[np.ndarray]:
        """
        Compute pool_3 image features for FID, IS, Precision, and Recall.

        :param batches: an iterator over NHWC numpy arrays in [0, 255].
        :return: a one-item tuple containing pool_3 features.
        """
        preds = []
        for batch in tqdm(batches):
            batch = batch.astype(np.float32)
            pred = self.sess.run(self.pool_features, {self.image_input: batch})
            preds.append(pred.reshape([pred.shape[0], -1]))
        return (np.concatenate(preds, axis=0),)

    def read_statistics(
        self, npz_path: str, activations: Tuple[np.ndarray]
    ) -> Tuple[FIDStatistics]:
