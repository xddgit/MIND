...
def qualitative_eval(args, cur_en_q_level=None):
    ...
    if args.quant_way == 'vq':
        ...
        tokenizer_model = FSQ_models[args.fsq_model](levels=args.levels)
    elif args.quant_way == 'lq':
        tokenizer_model = LQ_models[args.lq_model](levels=args.levels)
    else:
        raise ValueError("please check quant way")
    ...
    if args.quant_way == 'vq':
        ...
        save_folder_name = (f"{getattr(args, args.quant_way + '_model')}"
    ...
    if rank == 0:
        os.makedirs(sample_folder_dir, exist_ok=True)
        print(f"Saving outputs at {sample_folder_dir}")
    dist.barrier()
    ...
    loader = DataLoader(dataset, batch_size=args.per_proc_batch_size, shuffle=False, sampler=sampler, num_workers=args.num_workers, pin_memory=True)    
    ...
    all_tokens = []
    all_labels = []
    has_saved_check_images = False
    
    # 内存保护机制：每积累多少个样本保存一次 .npz 并清空内存
    # 50,000 个样本占用的纯 Token 内存仅几十MB，既不会太碎，也不会 OOM
    ...
    for x, y in loader: 
        ...
        with torch.no_grad():
            if args.quant_way == 'vq':
                latent, _, [_, _, indices] = tokenizer_model.encode(x, num_en_q_level=cur_en_q_level, causal_type=causal_type)
            elif args.quant_way in ['fsq', 'lq']:
                latent, indices, _ = tokenizer_model.encode(x)
                
            # --- 保存前向校验图 (仅第一个 Batch) ---
            if not has_saved_check_images:
                test_samples = tokenizer_model.decode_code(indices, latent.shape) if args.quant_way == 'vq' else tokenizer_model.decode_code(indices)
                test_samples = torch.clamp(127.5 * test_samples + 128.0, 0, 255).permute(0, 2, 3, 1).to("cpu", dtype=torch.uint8).numpy()
                
                check_dir = os.path.join(sample_folder_dir, "id_reconstruction_checks")
                if rank == 0: os.makedirs(check_dir, exist_ok=True)
                dist.barrier()
                
                for j in range(min(4, x.shape[0])):
                    Image.fromarray(test_samples[j]).save(os.path.join(check_dir, f"rank{rank}_id_recon_{j:02d}.png"))
                has_saved_check_images = True

            # ------------------------------------
            # 收集 Token ID 和 Label 到内存列表
            # ------------------------------------
            batch_tokens = indices.view(x.shape[0], -1).cpu().numpy()
            all_tokens.append(batch_tokens)
            all_labels.append(y.cpu().numpy())
            current_accumulated += x.shape[0]

            # ==============================================================================
            # [🔥 核心修改：触发分块落盘并清空内存]
            # ==============================================================================
            if current_accumulated >= SAVE_CHUNK_SIZE:
                tokens_np = np.concatenate(all_tokens, axis=0)
                labels_np = np.concatenate(all_labels, axis=0)
                
                npz_save_path = os.path.join(sample_folder_dir, f"imagenet_tokens_rank{rank}_part{chunk_index}.npz")
                np.savez(npz_save_path, tokens=tokens_np, labels=labels_np)
                
                # 打印日志以防用户焦虑
                if rank == 0:
                    print(f"\n[INFO] Auto-saving chunk {chunk_index} ({current_accumulated} samples) to clear memory...")
    ...
    if rank == 0:
        print(f"\n[INFO] Distributed NPZ extraction completed successfully!")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--per-proc-batch-size", type=int, default=96)
